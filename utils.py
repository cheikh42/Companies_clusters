...
def unsupervised_class(doc_ids: list, names: list, train_type: str, nb_workers: int):
    """
...
def tfidf_classification(names: list):
    ...
    cosine_matrix = awesome_cossim_topn(
        tfidf_matrix, tfidf_matrix.transpose(), names.size, 0.8
    )
...
def siren_mapping(list_siren_companies: list):
    ...
    dict_sirens = (
        dict()
    )  ### dict of sirens (keys) and the companies having these sirens (values).
